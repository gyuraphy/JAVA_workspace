...
public class TravelTest {
	...
	public static void main(String[] args) {
		...
		// 중간 연산, 최종연산을 이용해서 출력 함
		// 예1, 인원수 카운트
		// 예2, 비용이 특정 조건보다 많은 혹은 적은 사람 출력해보기
		...
		TravelCustomer customercdg= new TravelCustomer("CDG", 28, 300000);
		
		List<TravelCustomer> customerList = new ArrayList<>();
		customerList.add(customerLee);
		customerList.add(customerKim);
		customerList.add(customerHong);
		customerList.add(customercdg);
		...
		System.out.println("총 인원은 :" + count + "입니다");
		
		System.out.println("== 20세 이상 고객 명단 정렬하여 출력 ==");
		customerList.stream().filter(c->c.getAge() >= 20).map(c->c.getName()).sorted().forEach(s->System.out.println(s));
		
		System.out.println("== 비용이 제일 많은 사람 출력 ==");
		customerList.stream().filter(c->c.getPrice() >= 100001).map(c->c.getName()).sorted().forEach(s->System.out.println(s));
	}
}
